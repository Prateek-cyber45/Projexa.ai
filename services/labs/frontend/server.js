...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/healthz') {
    ...
    res.end(JSON.stringify({ service: 'labs-frontend', ok: true }));
    return;
  }

  // Block /api and /ws paths - these should be routed by nginx to backend
  if (pathname.startsWith('/api/') || pathname.startsWith('/ws/')) {
    res.writeHead(404, { 'Content-Type': 'application/json; charset=utf-8' });
    res.end(JSON.stringify({ error: 'Not Found', detail: 'API and WebSocket paths should be routed to backend' }));
    return;
  }
  ...
  if (pathname === '/simulator' || pathname.startsWith('/simulator/')) {
    sendFile(res, path.join(PUBLIC_DIR, 'simulator.html'));
    return;
  }
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`labs-frontend listening on port ${PORT}`);
});
